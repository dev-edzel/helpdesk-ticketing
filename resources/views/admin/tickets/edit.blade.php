...
<div class="card">
    ...
    <div class="card-body">
        <form action="{{ route("admin.tickets.update", [$ticket->id]) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group {{ $errors->has('emp_dept') ? 'has-error' : '' }}">
                <label for="emp_dept">{{ trans('cruds.ticket.fields.emp_dept') }}</label>
                <input type="text" id="emp_dept" name="emp_dept" class="form-control" value="{{ old('emp_dept', isset($ticket) ? $ticket->emp_dept : '') }}">
                @if($errors->has('emp_dept'))
                    <em class="invalid-feedback">
                        {{ $errors->first('emp_dept') }}
                    </em>
                @endif
                <p class="helper-block">
                    {{ trans('cruds.ticket.fields.emp_dept_helper') }}
                </p>
            </div>
